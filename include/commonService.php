...
class CommonService {
		...
		if (mysql_num_rows ( $result ) > 0) {
			...
			while ( $rows = mysql_fetch_array ( $result ) ) {
				...
				foreach ( $array_column as $value => $key ) {
					...
					} else if ($value == 'order_status' || $value == 'reservation_status' || $value=='new_status' || $value=='inout_status') {
						if ($rows [$value] == 'Y') {
			...
			while ( $rows = mysql_fetch_array ( $result ) ) {
				...
				foreach ( $array_column as $value => $key ) {
					if ($key == 'hidden_field') {
						...
						echo "<td><input type='checkbox' " . $rows [$value] . " onclick='toggleDivCheckBox(\"quantity_return_" . $counter_colum . "\");checkTheReturnCheckBox();' id='checkbox_return_" . $counter_colum . "'/></td>";
					} else if ($value == 'qtyre') {
						echo "<td><input type='number' style='width:30px;height:13px;display:none;margin-top:1px;' value='" . $rows ['quantity'] . "' id='quantity_return_" . $counter_colum . "' onclick='changeReturnQty(" . $counter_colum . ")'  onkeyup='changeReturnQty(" . $counter_colum . ")'>
	...
	function displayBg() {
		...
		echo " style='background: " . $color . "; color: " . $this->getContrastYIQ ( $color ) . ";min-height:40px;' ";
	}
	function displayColor($bg_color, $text_color) {
		echo " style='background: " . $bg_color . "; color: " . $text_color . "' ";
	}
