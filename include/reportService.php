...
class ReportService {
	...
	function generateDataCustomerChart($params, $chartId, $title, $nbrShop) {
		$datefrom = isset ( $params ['datefrom'] ) ? $params ['datefrom'] : date ( 'Y-m-01' );
		$dateto = isset ( $params ['dateto'] ) ? $params ['dateto'] : date ( 'Y-m-t' );
		$charttype = isset ( $params ['charttype'] ) ? $params ['charttype'] : 'spline';
		$charttime = isset ( $params ['charttime'] ) ? $params ['charttime'] : '%Y-%m-%d';
		...
		$str = "$(document).ready(function () {
            $('#" . $chartId . "').jqChart({
                ...
                series: [";
		$str = $str . $this->genDataCustomer ( $datefrom, $dateto, $charttype, $charttime, $nbrShop, $id_shop , $params['issimplechart']) . "]
            });
        });";
		echo $str;
	}
	...
	function genDataCustomer($datefrom, $dateto, $charttype, $charttime, $nbrShop, $id_shop,$issimplemode) {
		$returnStr = "";
		$returnStr = $returnStr . $this->generateCustomer ( $datefrom, $dateto, $charttype, $charttime, $id_shop,$issimplemode );
		return substr ( $returnStr, 0, - 1 );
	}
function generateCustomer($datefrom, $dateto, $charttype, $charttime, $shop_id, $issimplemode) {
		...
		'" . $datefrom . "' and '" . $dateto . "'  group by date_format(t1.date,'" . $charttime . "')";
		
		$qryTotalFactureGuess = "select count(*) as total, date_format(t1.date,'" . $charttime . "') as date 
		from export_facture t1 where t1.date between 
		'" . $datefrom . "' and '" . $dateto . "' and t1.customer_id =1288 group by date_format(t1.date,'" . $charttime . "')";
		
		$qryTotalFactureNoGuess = "select count(*) as total, date_format(t1.date,'" . $charttime . "') as date 
		from export_facture t1 where t1.date between 
		'" . $datefrom . "' and '" . $dateto . "' and t1.customer_id <> 1288 group by date_format(t1.date,'" . $charttime . "')";
...
//		echo date ( 'H' );
	...
	function getNbrCustomer($startdate, $enddate) {
		$qry = "";
		return $this->getAmountReportnoFormat ( $qry );
	}
