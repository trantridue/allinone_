...
CREATE TABLE IF NOT EXISTS `allinone`.`product_return` (
  ...
  `import_facture_code` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_product_return_import_facture1`
    FOREIGN KEY (`import_facture_code`)
    REFERENCES `allinone`.`import_facture` (`code`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `fk_product_return_import_facture1_idx` ON `allinone`.`product_return` (`import_facture_code` ASC);
...
START TRANSACTION;
USE `allinone`;
INSERT INTO `allinone`.`product` (`code`, `name`, `category_id`, `season_id`, `sex_id`, `export_price`, `description`, `brand_id`, `sale`) VALUES ('0001', 'Áo nam', 1, 1, 1, 1000, 'Helo', 1, 0);
INSERT INTO `allinone`.`product` (`code`, `name`, `category_id`, `season_id`, `sex_id`, `export_price`, `description`, `brand_id`, `sale`) VALUES ('0002', 'Quần jean', 2, 2, 2, 400, 'Tốt', 2, 0);
